      ...
      <div class="doc-card form-card">
        ...
        <div class="upload-wrapper">
          <a-upload list-type="text" :file-list="formFileList" :custom-request="(options) => handleFormUpload(options)"
            @before-remove="handleFormRemove" :disabled="formFileList.length >= 1"  draggable>
          </a-upload>
        </div>
      </div>
...
const submitAllFiles = (idLastSix: string) => {
  console.log('提交的身份证后六位:', idLastSix)
  console.log('图片类文件列表:', fileListMap)
  console.log('申请表文件列表:', formFileList.value)
